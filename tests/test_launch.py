...
class TestLaunchFunctions(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.conf = MagicMock()
        self.conf.n_gpu = 2
        self.conf.launch_config = LaunchConfig(min_nodes=1, max_nodes=1, nproc_per_node=2, run_id="test")
    ...
    def test_run_with_multiple_gpus(self, mock_elastic_launch):
        ...
        mock_elastic_launch.assert_called_once_with(config=self.conf.launch_config, entrypoint=elastic_worker)
        mock_elastic_launch.return_value.assert_called_once_with(self.fn, self.args, self.conf.n_gpu)
    ...
    def test_elastic_worker(self, mock_set_device, mock_create_local_process_group, mock_synchronize, mock_init_process_group, mock_device_count, mock_is_available):
